...
export function TrainerSetup({
  ...
  onStart: (run: RunSpec, selection: ScenarioSelection) => void;
}) {
  ...
  const [family, setFamily] = useState<ScenarioSelection>("mixed");
  ...
  const previewRun = selectedRun(drill, family);
  const castCount = previewRun.scenario.events.filter(
    (event) => event.kind === "defile",
  ).length;
  useEffect(() => {
    if (!preparing && restoreFocus.current) {
      restoreFocus.current = false;
      startButtonRef.current?.focus({ preventScroll: true });
    }
  }, [preparing, startButtonRef]);
  useEffect(
    () => () => {
      request.current++;
    },
  ...
  async function begin() {
    ...
    setError("");
    try {
      const run = selectedRun(drill, family);
      const errors = validateRun(run);
      if (errors.length)
        throw new Error("Invalid practice profile: " + errors.join("; "));
      await prepareScenarioAssets(run);
      if (id === request.current) onStart(run, family);
    } catch (cause) {
      if (id !== request.current) return;
      restoreFocus.current = true;
      setError(
        cause instanceof Error &&
          cause.message.startsWith("Invalid practice profile")
          ? cause.message
  ...
  return (
    ...
    >
      ...
      <div className="rt-quick-bar">
        ...
        <div className="rt-quick-field rt-scenario-field">
          <span id="rt-scenario-label">Scenario</span>
          <Select
            aria-label="Scenario"
            value={family}
            disabled={preparing}
            onValueChange={(value) => {
              setFamily(value as ScenarioSelection);
              setError("");
            }}
          >
            <SelectOption value="mixed">Mixed practice</SelectOption>
            {drill.families.map((entry) => (
              <SelectOption key={entry} value={entry}>
                {familyLabels[entry]}
              </SelectOption>
            ))}
          </Select>
        ...
        <div className="rt-quick-start">
          <span>
            {previewRun.scenario.duration} seconds · {castCount}{" "}
            {castCount === 1 ? "cast" : "casts"} · Focus mode
          </span>
          <button
            ref={startButtonRef}
      ...
      <div className="rt-mechanic-overview">
        <div className="rt-mechanic-copy">
          <h2>{drill.name}</h2>
          <p>{drill.summary}</p>
          <p className="rt-supporting-copy">
            Also active: {supportingMechanics(previewRun)}
          </p>
          <details className="rt-profile-details">
            <summary>Practice scope</summary>
            <p>{practiceScope}</p>
          </details>
