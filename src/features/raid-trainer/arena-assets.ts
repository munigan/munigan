...
import { createRoster } from "./scenario-content";
...
// The shared runtime owns all required art. Every roster class is decoded before GO.
export const scenarioArtwork = {
  arena: "/raid-trainer/art/arena-sculpted-ice.png",
  boss: "/raid-trainer/art/lich-king.jpg",
  valkyr: "/raid-trainer/art/valkyr.svg",
  spirit: "/raid-trainer/art/vile-spirit.svg",
};

export async function prepareScenarioAssets(run: RunSpec): Promise<void> {
  const paths = [
    ...Object.values(scenarioArtwork),
    "/raid-trainer/art/defile.jpg",
    "/raid-trainer/art/regroup.jpg",
    ...createRoster(run).map((actor) => actor.classIcon),
  ];
  await Promise.all([...new Set(paths)].map(loadImage));
}
