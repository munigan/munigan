...
import englishSettings from "../../../messages/en-US/settings.json";
import portugueseSettings from "../../../messages/pt-BR/settings.json";
...
function view(
  ...
  return (
    <NextIntlClientProvider
      locale={locale}
      messages={{
        settings: locale === "pt-BR" ? portugueseSettings : englishSettings,
...
it("preserves raw unsaved exports and translates visible errors when locale changes", () => {
  ...
  const view = (portuguese: boolean) => (
    <NextIntlClientProvider
      ...
      messages={{
        settings: portuguese ? portugueseSettings : englishSettings,
...
it("keeps the accepted profile, bags, preset and lookup after refresh fails", async () => {
  ...
  await user.click(
    await screen.findByRole("option", { name: "Fury (19/52/0)" }),
  );
  await waitFor(() =>
