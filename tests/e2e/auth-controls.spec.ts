...
async function mockAccountSession(page: Page, payload: AccountSessionFixture = authenticated) {
...
test("desktop account states keep one measured utility-slot width", async ({ page }) => {
  ...
  const unavailableWidth = (await page.locator(".auth-control").boundingBox())!.width;
  expect({ loadingWidth, authenticatedWidth, unavailableWidth }).toEqual({ loadingWidth: 184, authenticatedWidth: 184, unavailableWidth: 184 });
});

test("390px mobile navigation exposes account actions without overflow", async ({ browser }) => {
  const context = await browser.newContext({ viewport: { width: 390, height: 844 } });
  const page = await context.newPage();
  await mockAccountSession(page);
  await page.goto("/pt-br");
  await page.getByRole("button", { name: "Abrir navegação" }).click();
  await expect(page.getByRole("link", { name: "Minha Biblioteca" })).toBeVisible();
  await expect(page.getByRole("button", { name: "Sair" })).toBeVisible();
  await expect(page.getByRole("button", { name: "Excluir conta" })).toBeVisible();
  expect((await page.locator(".workbench-drawer").boundingBox())!.width).toBeLessThanOrEqual(390);
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
  await context.close();
});

test("390px anonymous sign-in dialog stays inside the viewport", async ({ browser }) => {
  const context = await browser.newContext({ viewport: { width: 390, height: 844 } });
  const page = await context.newPage();
