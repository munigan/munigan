...
// Draft writes are coalesced; pagehide is the production flush boundary.
...
test("real worker derives mixed-tier gear, replaces balances, restores exclusions and submits only purchase inputs", async ({
  ...
  // The selected Fury specialization includes DPS rewards, not tank variants.
  expect(shoulders).toEqual(["50082"]);
