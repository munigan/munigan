...
Imported fields, including explicit zero/false values and empty enhancements, take precedence over presets. Missing settings use the selected simulator preset. Simple/legacy rotations must be switched to Automatic or APL before import. Known profession ranks below 450 are blocked because the pinned engine assumes maximum profession bonuses.

All eligible imported items start selected. Unsupported bag entries remain visible in an icon grid and are excluded by default. Unchecking their exclusion blocks submission until it is checked again. Item icons/names show Wowhead Wrath tooltips with the original gems and enchant; local details remain available if Wowhead is blocked. Invalid equipped gear blocks submission. The allowance is an upper bound; the worker removes illegal or mechanically identical inputs before charging actual work. Paired-slot order is preserved.
