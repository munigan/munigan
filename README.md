...
Imported fields, including explicit zero/false values and empty enhancements, take precedence over presets. Missing settings use the selected simulator preset. Simple/legacy rotations must be switched to Automatic or APL before import. Known profession ranks below 450 block simulation because the pinned engine assumes maximum profession bonuses. They do not block draft restore: saved gear, selections and settings reopen for review, with imported ranks visible under Professions and readiness errors displayed beside Run.
