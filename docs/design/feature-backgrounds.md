...
Generated with the built-in image generation tool on 2026-09-11 and restored here from the existing `discord-auth` worktree. Final assets: `public/images/backgrounds/`, with 960px and 1672px AVIF/WebP variants. Source PNGs remain in that worktree's `.cache/background-sources/` and are not shipped. One representative race per specialization rather than exact imported race matching. Phase/rotation variants share artwork.
...
Naxxramas plague quarter, broad quiet surfaces, gothic arches far right, subtle green slime near the app action green (#78E34D), dim reflections on wet stone. Left 60% charcoal negative space; sparse environment, no characters, soft depth, bottom fade. Used on Gear Lab and report pages until the visible character's specialization is known.
...
The homepage uses its existing Icecrown artwork. All page backgrounds sit outside the content-width container, fill the viewport up to a centered 2400px maximum, and fade into the canvas at their sides and bottom. Gear Lab and report artwork uses 20% opacity on desktop and 14% on mobile, with a right-biased mobile crop. Other tools do not load these backgrounds.

Verified the Frost Death Knight import → review → equipment → return-to-import flow, report editing, and English/Portuguese homepages. Browser measurements at 390px, 1440px, and 3440px confirm the width cap and no horizontal overflow. The restored mapping covers every supported preset, and all 26 themes have both sizes in AVIF and WebP.
