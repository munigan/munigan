...
### Free Top Gear allowance and ranked sets

User clarification on 2026-09-08: free Top Gear must visibly limit selected combination work and report multiple best equipment combinations. Its ranking unit is a complete legal loadout, not an individual item. Keep the equipped reference, the highest simulated set, and alternative sets with exact DPS, gain versus equipped, swap count, complete slot/enhancement details, and uncertainty status. For small admitted searches retain every evaluated set; larger reports paginate retained results and state how many were evaluated, refined, and shown. Do not present coarse eliminated candidates as equally precise finalists or imply a partial list is every possible set.

Offer gear-difference views against equipped gear or the highest-DPS set; switching this view does not change the numerical gain reference, which remains equipped. Include jump-to-equipped, a full-set view, and reuse of a chosen set in another tool. A fewer-changes preference chooses a recommended set within the statistically tied group without reordering or altering exact DPS results. Preserve original owned inventory when reusing a set; it changes the equipped reference only after an explicit user action.

Limit combination work rather than a flat number of item checkboxes: two items in several slots can multiply into many loadouts. Show used/allowed budget and combination count beside Run, updating for selections, locks, and precision. Define a versioned allowance cost per set/precision and count any separate equipped reference once. The design example uses 300,000 budget units and 5,000 units per set (60 sets); these are provisional accounting values inspired by observed Raidbots UI, not a benchmarked WotLK production entitlement. They are not a promise that exactly 5,000 fights execute for each set. Track actual iterations separately and maintain independent hard runtime/iteration/global-compute budgets. Over-limit selections disable submission, preserve inventory/settings, and suggest narrowing slots or locking items; the server independently recomputes and rejects excess work before dispatch. No checkout or sign-in wall is introduced.
...
Follow-up on 2026-09-08 supersedes that final research limitation: completed one anonymous three-combination Top Gear run, inspected queue/processing, all three ranked sets, gear-difference toggle, fewer-changes preference, and per-set actions. Setup displayed 15,000 / 300,000 iterations for three combinations, while the completed report displayed 3,929 Smart Sim iterations and three actors. This confirms allowance accounting must be distinguished from measured iterations. The synthetic retail fixture produced item-level/enchant/model warnings; this is UI research, not validation of realistic character DPS or WotLK mechanics. See [run evidence and adaptation decisions](../../design/raidbots-top-gear-observations.md).
