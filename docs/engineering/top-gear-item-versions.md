...
Final verification: 34 unit/UI tests, 11 database integration tests, 45 native simulator tests, 5 source-data tests and all 5 end-to-end tests pass. Typecheck, lint, production build, scoped design contract, spec coverage and diff whitespace checks pass. Independent final review found no blocking or important correctness findings across source extraction, all 28 effect mappings, native profile isolation, Classic compatibility and admission reuse. Implementation commits: `f97701d` (native/data), `9f05da5` (selector/runtime/persistence).
